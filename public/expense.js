async function addExpense(event){ 
    try {
        ...
        const response = await axios.post("http://52.90.203.41:3000/expense/add-expense",obj, { headers: {"Authorization" : token} })
    ...
    window.addEventListener("DOMContentLoaded", async ()=>{
        try {
            ...
            const page=1;
            const response = await axios.get(`http://52.90.203.41:3000/expense/get-expense/${page}`,{ headers: {"Authorization" : token}} )
            console.log(response)
...
window.addEventListener("DOMContentLoaded", async()=>{
    try {
        ...
        const decodeToken = parseJwt(token)
        console.log(decodeToken);
        const response = await axios.get(`http://52.90.203.41:3000/expense/download-expense`,{ headers: {"Authorization" : token}} )
        console.log(response)
        for (var i=0; i<response.data.allExpenses.length;i++){
...
function displayfile(obj){
    ...
    async function deleteExpense(e,expenseId){
        try {
            const token = localStorage.getItem('token')
            const  response = await axios.delete(`http://52.90.203.41:3000/expense/delete-expense/${expenseId}`, { headers: {"Authorization" : token} })
    ...
    function showLeaderboard(){
        ...
        inputElement.onclick = async() => {
            const token = localStorage.getItem('token')
            const userLeaderBoardArray = await axios.get('http://52.90.203.41:3000/premium/showLeaderBoard', { headers: {"Authorization" : token} })
...
async function download(){
    try {
        const token = localStorage.getItem('token')
        const response = await axios.get('http://52.90.203.41:3000/expense/download',{ headers: {"Authorization" : token}} )
...
document.getElementById('rzp-button1').onclick = async function (e) {
    const token = localStorage.getItem('token')
    const response  = await axios.get('http://52.90.203.41:3000/purchase/premiummembership', { headers: {"Authorization" : token} });
    console.log(response);
    var options = {
        "key": response.data.key_id, 
        "order_id": response.data.order.id,
        "handler": async function (response) {
            const res = await axios.post('http://52.90.203.41:3000/purchase/updatetransactionstatus',{
    ...
    async function getExpenses(page){
        try {
            ...
            const response = axios.get(`http://52.90.203.41:3000/expense/get-expense/${page}`,{ headers: {"Authorization" : token}} )
            console.log(response)
            for (var i=0; i<response.data.allExpenses.length;i++){
                display(response.data.allExpenses[i])
            }
